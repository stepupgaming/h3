...
export function buildTemplate(withJev: H3JevMode = false): Graph {
  const g = workflow(`comfy-workflow-h3-t2va${h3JevPackageSuffix(withJev)}`);
  ...
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/t2va_av" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/t2va" });
  ...
  const conditioningZeroOut_7 = g.add(ConditioningZeroOut, {
  ...
  if (usesJevResMultistep(withJev)) {
    samples = addH3JevResMultistepSampler(g, {
      ...
      positive: unsafeRef(miniMaxH3ImageToVideo_6.id, 0) as NodeOutput<"CONDITIONING">,
      latent: unsafeRef(miniMaxH3ImageToVideo_6.id, 1) as NodeOutput<"LATENT">,
      seed,
      steps,
    });
  } else {
    const kSampler_8 = g.add(KSampler, {
      "seed": seed,
      "steps": steps,
      "cfg": 1,
      "sampler_name": "euler",
      ...
      "negative": unsafeRef(conditioningZeroOut_7.id, 0),
      "latent_image": unsafeRef(miniMaxH3ImageToVideo_6.id, 1),
    }, { id: "8" });
    samples = unsafeRef(kSampler_8.id, 0) as NodeOutput<"LATENT">;
  }
  const gemmyH3SaveAVLatent_9 = g.add(GemmyH3SaveAVLatent, {
    "filename_prefix": latent_prefix,
    "fingerprint": fingerprint,
    "samples": samples,
  }, { id: "9" });
  const vaeDecode_10 = g.add(VAEDecode, {
    "samples": samples,
    "vae": unsafeRef(vaeLoader_3.id, 0),
  }, { id: "10" });
  const vaeDecodeAudio_11 = g.add(VAEDecodeAudio, {
    "samples": samples,
    "vae": unsafeRef(vaeLoader_4.id, 0),
  }, { id: "11" });
  const createVideo_12 = g.add(CreateVideo, {
    "fps": 24,
    "images": unsafeRef(vaeDecode_10.id, 0),
    "audio": unsafeRef(vaeDecodeAudio_11.id, 0),
  }, { id: "12" });
  const saveVideo_13 = g.add(SaveVideo, {
    "filename_prefix": output_prefix,
    "format": "auto",
    "codec": "auto" as never,
    "video": unsafeRef(createVideo_12.id, 0),
  }, { id: "13" });
  g.output(gemmyH3SaveAVLatent_9.out(0));
  g.output(saveVideo_13.out(0));
  return g.toGraph();
}
...
export const manifestMeta = {
  name: "comfy-workflow-h3-t2va",
  title: "H3 Text to Video-Audio",
  description: "MiniMax-H3 FL2VA DiT text-to-video with audio. Gemmy `video h3 generate` T2VA path.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
