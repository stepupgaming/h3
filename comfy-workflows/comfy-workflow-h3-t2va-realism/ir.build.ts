...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-t2va-realism");
  ...
  const lora = g.param("lora", { type: "string", default: "h3-realism-people-t2v-i2v-r2v.safetensors" });
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/t2va_realism_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/t2va_realism" });
  ...
  const loraLoaderModelOnly_5 = g.add(LoraLoaderModelOnly, {
    "lora_name": lora,
    "strength_model": 1,
  ...
  const miniMaxH3SigmaShift_6 = g.add(MiniMaxH3SigmaShift, {
    ...
    "model": unsafeRef(loraLoaderModelOnly_5.id, 0),
  }, { id: "6" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-t2va-realism",
  title: "Comfy Workflow H3 T2va Realism",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-t2va-realism. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
