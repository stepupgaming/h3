...
export function buildTemplate(withJev: H3JevMode = false): Graph {
  const g = workflow(`comfy-workflow-h3-fl2va${h3JevPackageSuffix(withJev)}`);
  ...
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/fl2va_av" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/fl2va" });
  ...
  if (usesJevResMultistep(withJev)) {
    samples = addH3JevResMultistepSampler(g, {
      ...
      positive: unsafeRef(miniMaxH3ImageToVideo_7.id, 0) as NodeOutput<"CONDITIONING">,
      latent: unsafeRef(miniMaxH3ImageToVideo_7.id, 1) as NodeOutput<"LATENT">,
      seed,
      steps,
...
export const manifestMeta = {
  name: "comfy-workflow-h3-fl2va",
  title: "H3 First-Frame to Video-Audio",
  description: "MiniMax-H3 image-to-video (first frame). Gemmy `video h3 generate --mode i2v`.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
