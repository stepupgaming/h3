 ...
 *
 * Native-guide continue from saved RefMods (Text Encode, no live still).
 * Keyframes (tail) and refs (RefMods) are different conditioning keys.
 * Default UNET is Ref2VA. Do not Apply the same bundle.
 *
...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-continue-native-t2va-refmod");
  ...
  const empty = g.add(EmptyMiniMaxH3LatentAV, { width, height, length }, { id: "8" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-continue-native-t2va-refmod",
  title: "H3 Native-Guide Continue + RefMod (Text Encode)",
  description:
    "Native-guide continue from saved RefMods (Text Encode, no live still). Default UNET is Ref2VA.",
  outputs: [
