...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-continue-vae-tail");
  ...
  const context_video = g.param("context_video", { type: "string" });
  const context_frames = g.param("context_frames", { type: "int", default: 39 });
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/vae_tail_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/vae_tail" });
  ...
  const gemmyH3MaskedAVContext_9 = g.add(GemmyH3MaskedAVContext, {
    ...
    "tail_image": unsafeRef(getVideoComponents_8.id, 0),
    "tail_audio": unsafeRef(getVideoComponents_8.id, 1),
  }, { id: "9" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-continue-vae-tail",
  title: "Comfy Workflow H3 Continue Vae Tail",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-continue-vae-tail. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
