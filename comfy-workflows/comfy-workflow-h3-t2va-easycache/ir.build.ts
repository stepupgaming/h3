...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-t2va-easycache");
  ...
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/t2va_easy_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/t2va_easy" });
  ...
  const easyCache_5 = g.add(EasyCache, {
    "reuse_threshold": 0.2,
    "start_percent": 0.15,
    "end_percent": 0.95,
  ...
  const miniMaxH3SigmaShift_6 = g.add(MiniMaxH3SigmaShift, {
    ...
    "model": unsafeRef(easyCache_5.id, 0),
  }, { id: "6" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-t2va-easycache",
  title: "Comfy Workflow H3 T2va Easycache",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-t2va-easycache. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
