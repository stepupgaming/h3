# H3 Text to Video-Audio

MiniMax-H3 FL2VA DiT text-to-video with audio. Gemmy `video h3 generate` T2VA path.

Unofficial project. Not affiliated with or endorsed by Comfy Org.

## Purpose

Generate a video+audio clip from a text prompt using MiniMax-H3.
...
```sh
pnpm add @stepupgaming/comfy-workflow-h3-t2va
```
...
- `latent_prefix` (default "h3/t2va_av")
- `output_prefix` (default "h3/t2va")
- `fingerprint` (default "{}")
...
```sh
cwf inspect @stepupgaming/comfy-workflow-h3-t2va --url http://127.0.0.1:8188
cwf run @stepupgaming/comfy-workflow-h3-t2va --url http://127.0.0.1:8188 --param prompt=...
```
