 ...
 *
 * Native latent continuation: previous AV tail as MiniMaxH3AddGuide keyframes.
 * No denoise mask, no protected prefix trim. GemmyH3LatentTailGuide is rawNode
 * until environments/h3 is recaptured.
 */
...
import { addLatentTailGuide } from "../src/h3.ts";
import {
  CLIPLoader,
  ConditioningZeroOut,
  CreateVideo,
...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-continue-native-t2va");
...
export const manifestMeta = {
  name: "comfy-workflow-h3-continue-native-t2va",
  title: "H3 Native-Guide Continue (T2VA)",
  description: "Continue an H3 T2VA clip from a persisted AV latent via native keyframe guide (no denoise mask).",
  outputs: [
    { name: "output-0", type: "IMAGE" },
    { name: "output-1", type: "IMAGE" },
  ],
  models: [
    { kind: "model", name: "minimax_h3_fl2va_pruned_int8_convrot.safetensors" },
