...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-continue-t2va");
  ...
  const gemmyH3MaskedAVContext_8 = g.add(GemmyH3MaskedAVContext, {
    "context_frames": context_frames,
    "audio_mode": "generated_audio",
    "target": unsafeRef(miniMaxH3ImageToVideo_6.id, 1),
    "vae": unsafeRef(vaeLoader_3.id, 0),
    "audio_vae": unsafeRef(vaeLoader_4.id, 0),
    "previous": unsafeRef(gemmyH3LoadAVLatent_7.id, 0),
  }, { id: "8" });
  ...
  const kSampler_10 = g.add(KSampler, {
    ...
    "latent_image": unsafeRef(gemmyH3MaskedAVContext_8.id, 0),
  }, { id: "10" });
  ...
  const gemmyH3TrimProtectedPrefix_14 = g.add(GemmyH3TrimProtectedPrefix, {
    "images": unsafeRef(vaeDecode_12.id, 0),
    "protected_frames": unsafeRef(gemmyH3MaskedAVContext_8.id, 1) as never,
    "audio": unsafeRef(vaeDecodeAudio_13.id, 0),
  }, { id: "14" });
  const createVideo_15 = g.add(CreateVideo, {
    "fps": 24,
    "images": unsafeRef(gemmyH3TrimProtectedPrefix_14.id, 0),
    "audio": unsafeRef(gemmyH3TrimProtectedPrefix_14.id, 1),
  }, { id: "15" });
  const saveVideo_16 = g.add(SaveVideo, {
    "filename_prefix": output_prefix,
    "format": "auto",
    "codec": "auto" as never,
    "video": unsafeRef(createVideo_15.id, 0),
  }, { id: "16" });
  g.output(gemmyH3SaveAVLatent_11.out(0));
  g.output(saveVideo_16.out(0));
  return g.toGraph();
}
...
export const manifestMeta = {
  name: "comfy-workflow-h3-continue-t2va",
  title: "H3 Masked-AV Continue (T2VA)",
  description: "Continue an H3 T2VA clip from a persisted AV latent without a first-frame image.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
