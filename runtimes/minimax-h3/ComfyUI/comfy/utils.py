    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        try:
            if comfy.memory_management.aimdo_enabled:
                ...
                # `--disable-mmap` used to still call safe_open's default mmap
                # backend. On Windows that is CreateFileMapping WRITECOPY, which
                # charges the pagefile for the whole file (14GB TE -> OS error
                # 1455) and is the dirty-write path we are trying to avoid.
                # `backend=pread` reads into anonymous RAM. No file-backed COW,
                # no extra .to(copy=True) clone.
                open_kwargs = {"framework": "pt", "device": device.type}
                if DISABLE_MMAP:
                    open_kwargs["backend"] = "pread"
                with safetensors.safe_open(ckpt, **open_kwargs) as f:
                    sd = {}
                    for k in f.keys():
