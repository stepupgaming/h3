# H3 Masked-AV Continue (T2VA)

Continue an H3 T2VA clip from a persisted AV latent without a first-frame image.

Unofficial project. Not affiliated with or endorsed by Comfy Org.

## Purpose

Extend a T2VA generation with masked AV context.
...
```sh
pnpm add @stepupgaming/comfy-workflow-h3-continue-t2va
```
...
```sh
cwf inspect @stepupgaming/comfy-workflow-h3-continue-t2va --url http://127.0.0.1:8188
cwf run @stepupgaming/comfy-workflow-h3-continue-t2va --url http://127.0.0.1:8188 --param prompt=...
```
