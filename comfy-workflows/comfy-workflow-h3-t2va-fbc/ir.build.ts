...
export function buildTemplate(): Graph {
  const g = workflow("comfy-workflow-h3-t2va-fbc");
  ...
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/t2va_fbc_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/t2va_fbc" });
  ...
  const applyMiniMaxH3FirstBlockCache_5 = g.add(ApplyMiniMaxH3FirstBlockCache, {
  ...
  const miniMaxH3SigmaShift_6 = g.add(MiniMaxH3SigmaShift, {
    ...
    "model": unsafeRef(applyMiniMaxH3FirstBlockCache_5.id, 0),
  }, { id: "6" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-t2va-fbc",
  title: "Comfy Workflow H3 T2va Fbc",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-t2va-fbc. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
