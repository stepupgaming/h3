...
export function buildTemplate(withJev: H3JevMode = false): Graph {
  const g = workflow(`comfy-workflow-h3-fl2va-last${h3JevPackageSuffix(withJev)}`);
  ...
  const last_image = g.param("last_image", { type: "combo" });
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/fl2va_last_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/fl2va_last" });
  ...
  const miniMaxH3ImageToVideo_8 = g.add(MiniMaxH3ImageToVideo, {
    ...
    "last_frame": unsafeRef(loadImage_7.id, 0),
  }, { id: "8" });
...
export const manifestMeta = {
  name: "comfy-workflow-h3-fl2va-last",
  title: "Comfy Workflow H3 Fl2va Last",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-fl2va-last. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
