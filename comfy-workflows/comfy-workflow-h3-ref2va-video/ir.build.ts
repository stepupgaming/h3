...
export function buildTemplate(withVsa = false): Graph {
  const g = workflow(`comfy-workflow-h3-ref2va-video${withVsa ? "-vsa" : ""}`);
  ...
  const ref_video = g.param("ref_video", { type: "string" });
  const latent_prefix = g.param("latent_prefix", { type: "string", default: "h3/ref2va_video_av" });
  const fingerprint = g.param("fingerprint", { type: "string", default: "{}" });
  const output_prefix = g.param("output_prefix", { type: "string", default: "h3/ref2va_video" });
  ...
  g.connectInput(miniMaxH3ReferenceToVideo_8, "ref_videos.ref_video_0", unsafeRef(getVideoComponents_7.id, 0));
  g.connectInput(miniMaxH3ReferenceToVideo_8, "ref_video_audios.ref_video_audio_0", unsafeRef(getVideoComponents_7.id, 1));
...
export const manifestMeta = {
  ...
  title: "Comfy Workflow H3 Ref2va Video",
  description: "Imported ComfyUI workflow packaged as @stepupgaming/comfy-workflow-h3-ref2va-video. Verify license and redistribution rights before publishing.",
  outputs: [
  {
    "name": "output-0",
    "type": "IMAGE"
