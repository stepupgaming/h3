...
def minimax_block_lowmem_forward(self, x, t_emb, mod_segments, rope_freqs, transformer_options={}, attention=None):
    ...
    # BlockSparseAttention calls this with attention= set; that callable wants the tensor, not the list.
    shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaln_proj(t_emb)
    normed = _mod_scale_shift(self.norm1(x), shift_msa, scale_msa, mod_segments)
    if attention is None:
        attn_out = self.attn([normed], rope_freqs=rope_freqs, transformer_options=transformer_options)
    else:
        attn_out = attention(normed, rope_freqs=rope_freqs, transformer_options=transformer_options)
